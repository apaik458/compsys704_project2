 ...
 * @file    main.c
 ...
 1. BLE and USB-CDC Serial Debug is already done for you.
 	 1.1 BLE - When connected to the STM BLE Sensor App - data in 9 global variables :
 ...
 2. Initialize all sensors, variables, etc in the Block ***Initialize Here***
 ...
 4. The device name seen by the ST BLE Sensor app is set by the NodeName[1] - NodeName[7] variables, change them in ***Initialize here**** block
 ...
 6. startAcc and startMag are empty functions to initialize acc and mag sensors - use hints in these functions
...
void binprintf8(uint8_t v) {
	uint8_t mask=1<<((sizeof(uint8_t)<<3)-1);
	while(mask) {
		XPRINTF("%d", (v&mask ? 1 : 0));
		mask >>= 1;
	}
}

void binprintf16(uint16_t v) {
	uint16_t mask=1<<((sizeof(int16_t)<<3)-1);
	while(mask) {
		XPRINTF("%d", (v&mask ? 1 : 0));
		mask >>= 1;
	}
...
static void startMag() {
	...
	// Select operating mode (continuous mode) and power mode (HR mode)
...
static void startAcc() {
	...
	// Enable axes and select power mode (HR mode)
...
//		BoardName[i] = NodeName[i+1];
	...
	XPRINTF("SERVER: BLE Stack Initialized \r\n"
			...
			(hwVersion & 0x0F), fwVersion >> 8, (fwVersion >> 4) & 0xF,
			('a' + (fwVersion & 0xF)), BoardName, bdaddr[5], bdaddr[4],
